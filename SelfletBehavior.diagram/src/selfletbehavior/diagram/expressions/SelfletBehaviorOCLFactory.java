package selfletbehavior.diagram.expressions;
 ...
 * @generated
 */
public class SelfletBehaviorOCLFactory {

	/**
	 * @generated
	 */
	private final SelfletBehaviorAbstractExpression[] expressions;

	/**
	 * @generated
	 */
	protected SelfletBehaviorOCLFactory() {
		this.expressions = new SelfletBehaviorAbstractExpression[9];
	}

	/**
	 * @generated
	 */
	public static SelfletBehaviorAbstractExpression getExpression(int index,
			EClassifier context, Map<String, EClassifier> environment) {
		SelfletBehaviorOCLFactory cached = SelfletBehaviorDiagramEditorPlugin
				.getInstance().getSelfletBehaviorOCLFactory();
		if (cached == null) {
			SelfletBehaviorDiagramEditorPlugin.getInstance()
					.setSelfletBehaviorOCLFactory(
							cached = new SelfletBehaviorOCLFactory());
		}
		...
		if (cached.expressions[index] == null) {
			final String[] exprBodies = new String[] {
					...
					"self.state -> select (s | s.oclIsTypeOf(Init)) -> size() <= 1", //$NON-NLS-1$
					"self.targetBehavior.state -> select (s | s.oclIsTypeOf(Init)) -> size() <= 1", //$NON-NLS-1$
					"self.targetState.targetBehavior.targetService.name.concat(\'.\').concat(self.targetState.targetBehavior.name).concat(\'.\').concat(self.targetState.name).concat(\'.action\')", //$NON-NLS-1$
					"\'do /\'.concat(self.targetState.targetBehavior.targetService.name).concat(\'.\').concat(self.targetState.targetBehavior.name).concat(\'.\').concat(self.targetState.name).concat(\'.action\')", //$NON-NLS-1$
			};
	 ...
	 * @generated
	 */
	public static SelfletBehaviorAbstractExpression getExpression(String body,
			EClassifier context, Map<String, EClassifier> environment) {
	 ...
	 * @generated
	 */
	private static class Expression extends SelfletBehaviorAbstractExpression {

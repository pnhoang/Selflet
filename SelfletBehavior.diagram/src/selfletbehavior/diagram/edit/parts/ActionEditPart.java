...
import selfletbehavior.diagram.edit.policies.ActionItemSemanticEditPolicy;
...
public class ActionEditPart extends CompartmentEditPart implements
			...
			parser = SelfletBehaviorParserProvider
					.getParser(
							...
							SelfletBehaviorVisualIDRegistry
									.getType(selfletbehavior.diagram.edit.parts.ActionEditPart.VISUAL_ID));
		}
		return parser;
	}
	...
	protected void handleNotificationEvent(Notification event) {
		...
		if (event.getNotifier() instanceof Action) {
			...
			if ("abilityFile".equals(attrName) || "body".equals(attrName)) {
				...
				IEditorPart  editorPart =
					selfletbehavior.diagram.part.SelfletBehaviorDiagramEditorPlugin.getInstance().getWorkbench().getActiveWorkbenchWindow().getActivePage().getActiveEditor();
				...
				if(editorPart  != null)
				{
					...
					System.out.println("active Project Name: " + activeProjectName);

					String activeProjectLocation = activeProject.getLocationURI().toString();
					System.out.println("active Project Location: " + activeProjectLocation);
					...
					String filePath = activeProjectLocation.toString().replace("file:", "")
					+ "/actions/" + actionFileString;
